...
enable / allow / expose 三段门禁控制，**引入依赖不等于启用**。插件的主要使用方是
**开发者基于 `ai4j` / `ai4j-agent` 等 SDK 自研的 agent 系统** —— 官方 CLI / TUI /
Spring Boot starter 只是宿主的几种形态之一，插件必须对任意自研宿主同样可用。
本仓库由 AI4J 项目官方维护：插件经 review 收录、CI 验证，以
`io.github.lnyo-cly.community` groupId 统一发布到 Maven Central —— 对用户是可发现、
可审计的插件入口，对作者则无需自建发布管线。
...
- 宿主无关性：插件不得假设宿主有终端、UI 或交互面（禁用 `System.console`、GUI、直接阻塞式
  用户提问）。需要用户输入时走 tool/approval 机制，由宿主决定如何呈现 —— 插件要对任意
  基于 SDK 自研的宿主可用，不限于官方 CLI / TUI / Spring Boot / 无头 worker
